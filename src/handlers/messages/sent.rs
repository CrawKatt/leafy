...
pub async fn handler(ctx: &serenity::Context, new_message: Message) -> CommandResult {
    ...
    if let Err(why) = attachment_handler(new_message.clone()).await {
    ...
    if user_id.is_some() {
        handle_user_id(ctx, &new_message, guild_id, data.clone(), user_id).await?;
    }
...
async fn handle_user_id(
    ...
    let forbidden_user_id = load_data()
        .forbidden
        .user
        .parse::<UserId>()?;
    ...
    let forbidden_role_id = load_data()
        .forbidden
        .role
        .parse::<RoleId>()?;

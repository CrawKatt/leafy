...
use crate::utils::handlers::misc::link_spam_handler::{extract_link, handle_blacklist_link};
...
pub async fn message_handler(ctx: &serenity::Context, new_message: &Message) -> CommandResult {
    ...
    if let Some(link) = extract_link(&new_message.content) {
        handle_blacklist_link(ctx, new_message, guild_id, link, &data, &admin_role_id, time).await?;
    }

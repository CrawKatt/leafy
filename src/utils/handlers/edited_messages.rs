...
pub async fn edited_message_handler(ctx: &serenity::Context, event: &MessageUpdateEvent) -> Result<(), Error> {
    ...
    let new_content = event.content.as_deref().unwrap_log("No se pudo obtener el contenido del mensaje", current_line, current_module)?;
    ...
    let log_channel = log_channel_id.unwrap_log("No se pudo obtener el canal de Logs", current_line, current_module)?.log_channel_id;
    ...
    let user_mentioned = user.to_user(&ctx.http).await.unwrap_log("No se pudo obtener el usuario", current_line, current_module)?;
    let forbidden_user_data = ForbiddenUserData::new(user_mentioned.clone(), user, database_message.guild_id.unwrap_log("No se pudo obtener el id del servidor", current_line, current_module)?);
    let forbidden_user_id = forbidden_user_data.user_id;
    let forbidden_role_id = ForbiddenRoleData::get_role_id(database_message.guild_id.unwrap_log("No se pudo obtener el id del servidor", current_line, current_module)?).await?;
    let forbidden_role_id = forbidden_role_id.unwrap_log("No se pudo obtener el id del rol", current_line, current_module)?;
    let mentioned_user = database_message.guild_id.unwrap_log("No se pudo obtener el id del servidor", current_line, current_module)?.member(&ctx.http, user_id).await?;
    let mentioned_user_roles = mentioned_user.roles(&ctx.cache).unwrap_log("Could not get the roles", current_line, current_module)?;
    ...
    matches!(
        contains_forbidden_user, {
            ...
            handle_forbidden_user(ctx, &message, database_message.guild_id.unwrap_log("No se pudo obtener el id del servidor", current_line, current_module)?,database_message, forbidden_user_id).await?;
        },
        contains_forbidden_role, {
            let message = ctx.http.get_message(database_message.channel_id, database_message.message_id).await?;
            handle_forbidden_role(ctx, &message, database_message.guild_id.unwrap_log("No se pudo obtener el id del servidor", current_line, current_module)?,database_message).await?;
        },
        default, {

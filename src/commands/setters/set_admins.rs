...
pub async fn set_admins(
    ...
    let Some(admin_role) = admin_role else {
        ...
        let role = role.unwrap_log("Could not get the role", line!(), module_path!())?;
    ...
    let role_id = role.as_ref().map(|role| role.id);
    let role_2_id = role_2.as_ref().map(|role| role.id);
    let admin_data = AdminData::new(role.clone().unwrap_log("Could not get the role", line!(), module_path!())?, role_2.clone(), role_id, role_2_id, guild_id);
    admin_data.save_to_db().await?;

    let role_name = role.unwrap_log("Could not get the role_name", line!(), module_path!())?.name;
    ctx.say(format!("Admin role set to: **{role_name}**")).await?;
    
    let role_2 = role_2.unwrap_log("Could not get the role", line!(), module_path!())?;
